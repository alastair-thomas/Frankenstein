...
SVMCorpus <- function(traindata, testdata, cost, gamma) {
  x <- NULL
  y <- NULL
  for (i in 1:length(traindata)) {
    x <- rbind(x,traindata[[i]])
  ...
  colnames(dat) <- c(1:ncol(x),'y')
  
  svmfit <- svm(y~., data=dat, kernel="radial", cost=cost, gamma=gamma,scale=TRUE,class.weights='inverse')
  
  preds <- numeric(nrow(testdata))
  for (i in 1:nrow(testdata)) {
    df <- data.frame(x=testdata[i,,drop=FALSE])
    colnames(df) <- c(1:ncol(x))
    preds[i] <- predict(svmfit,df)
  }
  return(preds)
}
